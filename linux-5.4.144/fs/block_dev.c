...
static void blkdev_bio_end_io(struct bio *bio)/*gql-回调函数，bio的回调函数，在此将bio的返回数据传递给kiocb*/
{
	...
	if (!dio->multi_bio || atomic_dec_and_test(&dio->ref)) {
		if (!dio->is_sync) {
			...
			printk("012: blkdev_bio_end_io: bio->usrflag --iocb: %llu\n",bio->bi_usrflag);
...
__blkdev_direct_IO(struct kiocb *iocb, struct iov_iter *iter, int nr_pages)
{
	...
	*/
	...
	for (;;) {
		...
		bio->bi_ioprio = iocb->ki_ioprio;
		printk("003->__blkdev_direct_IO: iocb->ki_usrflag--bio: %llu\n",iocb->ki_usrflag);
		...
		if (unlikely(ret)) {
			...
			bio_endio(bio);/*gql-唯一能够调用回调函数的地方-ps(在direct-io的路径下*/
			break;
		}
...
ssize_t blkdev_read_iter(struct kiocb *iocb, struct iov_iter *to)/*gql- :读请求在块设备fs层的初始处理*/
{
	struct file *file = iocb->ki_filp;
	struct inode *bd_inode = bdev_file_inode(file);
	loff_t size = i_size_read(bd_inode);
